...
- f09b009: feat: surface stale embeddings (new `embeddingStatus: 'stale'` value + dashboard rebuild surface)
...
- c430d82: Generalise graceful shutdown coordination to all fire-and-forget background work (ADR-0024 supersedes ADR-0023).
...
- f0fbfd0: Fix `pack.install` to fire post-write hooks (conflict detection + auto-embed) for every freshly-written memory.
