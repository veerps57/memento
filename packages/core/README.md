...
| Export                                           | Purpose                                                                                                                                                                                                                                                    |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `reembedAll(repo, provider, options)`            | Bulk driver: walks active memories, identifies missing or stale (different `model` / `dimension`) rows, writes fresh embeddings via `MemoryRepository.setEmbedding`. Per-row transactions; provider errors are recorded as skips and don't halt the batch. |
| `EmbeddingProvider`                              | The contract every embedder satisfies — `model`, `dimension`, `embed(text)`. The local implementation lives in `@psraghuveer/memento-embedder-local`.                                                                                                                  |
