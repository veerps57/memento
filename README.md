...
| [`@psraghuveer/memento-embedder-local`](packages/embedder-local) | Local `EmbeddingProvider` backed by transformers.js + `bge-small-en-v1.5`. Lazy single-flight init; consumers install `@huggingface/transformers` only when they opt in to vector retrieval. See ADR [0006](docs/adr/0006-local-embeddings-only-in-v1.md).                                                                                                             |
