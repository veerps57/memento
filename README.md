...
That's it. Memento's MCP server emits a [session-start teaching spine](packages/server/src/instructions.ts) on every `initialize` handshake (ADR-0026). The MCP spec leaves the spine optional on the client side: clients like Claude Code that surface it inject the contract — when to load context, when to write, when to confirm — into the assistant's system prompt with no further wiring. Clients that ignore the field (Claude Chat / `claude.ai` web does today; behaviour varies across Cursor, VS Code, Cline) get the schema-embedded session-start hints instead. The bundled skill (when your client loads Anthropic-format skills) layers on the deeper distillation curriculum on intent. The persona snippet in [docs/guides/teach-your-assistant.md](docs/guides/teach-your-assistant.md) is the **universal fallback** that works in every client — paste it into your client's custom-instructions or system-prompt slot if your client doesn't honour the other two surfaces.
