...
When `retrieval.vector.enabled = true` **and** an `EmbeddingProvider` is wired into the host, candidates are also generated by cosine similarity over `embedding`. Memento ships `@memento/embedder-local` (`bge-small-en-v1.5` via transformers.js, lazy-loaded on first use); cloud embedders are out of scope (ADR 0006). Setting the flag without wiring a provider raises a structured `CONFIG_ERROR` at the first search — silent degradation to FTS-only would hide a real misconfiguration.

When vector search is enabled but a memory has no embedding (e.g., written before vector search was turned on), it is included only via FTS until `memento embedding rebuild` materialises its embedding. If a row's stored embedding model or dimension drifts from the configured provider, the search aborts with a `CONFIG_ERROR` pointing at `memento embedding rebuild` rather than mixing vector spaces (Rule 14: model migration must be deliberate).
