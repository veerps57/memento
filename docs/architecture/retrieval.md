...
When `retrieval.vector.enabled = true` **and** an `EmbeddingProvider` is wired into the host, candidates are also generated by cosine similarity over `embedding`. Memento ships `@psraghuveer/memento-embedder-local` (`bge-small-en-v1.5` via transformers.js, lazy-loaded on first use); cloud embedders are out of scope (ADR 0006). Setting the flag without wiring a provider raises a structured `CONFIG_ERROR` at the first search — silent degradation to FTS-only would hide a real misconfiguration.
