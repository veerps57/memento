...
### P1 — The developer (human user)

A working software engineer who already uses one or more AI coding
assistants. They install Memento because re-explaining themselves at the
start of every session is exhausting and because the per-tool memory
stores (one assistant's "rules file," another's "project memory," a third's
"custom instructions") fragment knowledge they consider their own.
...
### P2 — The AI coding assistant (agent user)

An LLM-driven assistant — a desktop chat client, an in-IDE agent, a
headless coding bot — that calls Memento on the developer's behalf. It
does not have eyes; it consumes structured input/output and reacts to
structured errors. Its goals are quality of context (recall what's
relevant, ignore what isn't), safety (don't write nonsense, don't lose
data, don't violate the developer's expectations), and self-correction
...
## P1 — Developer stories

### P1.1 Continuity across sessions

> As a developer, I want my AI assistant to remember decisions, preferences,
> and project conventions between sessions, so that I don't re-explain
> myself every time I open a new chat.
...
> As a developer, I want the same memory to be available regardless of
> which AI assistant I am using today, so that switching tools (or trying
> a new one) does not cost me my accumulated context.
...
> As a developer, I want my memory to live on my machine and never leave
> it without my explicit action, so that I can use this with sensitive
> work without re-evaluating a vendor's data policy.
...
> As a developer, I want my memory to be portable in a documented,
...
> As a developer, I want to see exactly what an assistant has stored
> about me, when, and why, so that I can trust — or correct — the
> memory layer.
...
> As a developer, I want to forget, archive, or replace any individual
...
> As a developer, I want bulk forget/archive/clear operations to default
> to a preview, so that I never lose data because I mistyped a filter.
...
> As a developer, I want common secrets to be redacted before they are
...
> As a developer, I want to control what counts as a secret and to be
> told when redaction altered something, so that I can adjust the
> defaults to my context (work vs. personal, strict vs. permissive).
...
> As a developer, I want memory to be scoped — a work preference about
> Repo A should not leak into a personal side project — so that
> recalled context is relevant and not noisy.
...
> As a developer, I want broader-context memory (about me as a person)
> to remain visible inside narrower contexts (a specific repo), so
> that "I prefer four-space indentation" is recalled everywhere it
> applies.
...
> As a developer, I want to install and verify Memento in under a
> minute, so that I'll actually try it instead of putting it off.
...
> As a developer, I want failure modes to be surfaced in writing, so
> that I don't waste an afternoon assuming a feature works when it
> doesn't.
...
> As a developer, I want richer features (e.g. paraphrase-aware
> recall) to be opt-in, so that the default install stays minimal and
> dependency-light.
...
> As a developer, I want to carry my memory from one machine to
> another, so that switching laptops, reinstalling an OS, or working
> from a second workstation does not cost me the context I have
> accumulated.
...
developer perceives the system as useful.
