...
4. **`OwnerRef` is always populated**, even when it is always `{ type: 'local', id: 'self' }` in v1. The model is multi-user-ready from day one.
...
11. **`MemoryEvent` is the audit source of truth.** `lastConfirmedAt` on `Memory` is a denormalized cache, validated at write-time and — once shipped — by the planned `memento doctor` command.
...
`pnpm verify` mirrors the CI gate. If it passes locally, CI on the same commit should pass too (modulo OS-specific surprises in the matrix). The individual scripts (`pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm docs:lint`, `pnpm docs:check`, `pnpm build`) remain available for tighter inner loops while iterating.

Dedicated contract and property test suites (`pnpm test:contract`, `pnpm test:property`) are tracked in [`KNOWN_LIMITATIONS.md`](KNOWN_LIMITATIONS.md) and will become mandatory once they land. Until then, contract-style coverage is folded into the unit suites and property invariants are checked by example tests; `pnpm test` is the full required test gate.
...
- **Do not import from `@memento/core` private paths.** Use the public exports from each package's `index.ts`. Private paths are an implementation detail.
...
These are deliberate omissions. Do not add them in v1. The architecture leaves the door open for v2.

- Cloud embedding providers (OpenAI, etc.) — v1 is fully local.
- Plugin / extension system. The `plugin.*` config namespace is reserved.
- Multi-user / team-scoped memory. The data model supports it; v1 code does not.
- HTTP / SSE transport for MCP. stdio only in v1.
- Encryption at rest.
- Sync across machines.
- Single-binary distribution.
- Web UI / TUI.

If you believe v1 needs one of these, open a [design proposal](.github/ISSUE_TEMPLATE/design_proposal.yml) — do not just implement it.
